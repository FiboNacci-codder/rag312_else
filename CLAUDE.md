...
Dependencies: `requirements.txt` is a pinned reference list (includes `docling`, `langchain*`, `qdrant-client`, `fastapi`/`uvicorn`, `gradio`, `ragas`), but installing it wholesale (`pip install -r requirements.txt` or a plain `pip install -e .`) breaks on `torch==2.6.0+cu124`/`torchaudio`/`torchvision`, whose `+cu124` wheels only exist on `https://download.pytorch.org/whl/cu124`, not PyPI — and the conda base env on `elsecluster5` ships CUDA 13 torch by default, not the 12.4 this project needs. No separate lint/format tooling is configured in this repo.

Instead, run `pip install -e . --no-deps` (from the repo root) to install the local `rag312/` package in editable mode without pulling in the full pin list — it's the only way `ingesta/`, `consulta/`, `interfaz/`, and `tests/eval/` scripts can `import rag312`. Re-run it whenever `pyproject.toml` changes. Then fix torch explicitly (`pip uninstall torch torchvision torchaudio -y && pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu124`) and install remaining runtime deps incrementally as scripts report `ModuleNotFoundError`. See `docs/ejecutar.md` §3 for the full step-by-step.
